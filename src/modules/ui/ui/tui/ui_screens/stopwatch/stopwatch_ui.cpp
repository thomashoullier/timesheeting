...
namespace tui {
  ...
  bool StopwatchUI::commit_entrystaging() {
    core::EntryStaging entry_staging = db::db().query_entrystaging();
    auto current_start = entry_staging.start;
    if (current_start.has_value() and
        !day_selector->get_selected_day().contains(current_start.value())) {
      status().print_wait("Adding entries to other days is forbidden.");
      return false;
    }
