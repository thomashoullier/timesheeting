...
*** TODO Document the timesheet export format
ADDED: [2024-10-26 Sat]
...
+ Project ID are unique.
+ Task names are unique per project.
+ Task ID are unique.
+ No field may be empty.
+ Location id are unique
+ Location names are unique
+ Stop date is guaranteed to be after start date in entries.
