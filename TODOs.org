...
+ We must use the UpdateManager to signal to screens that they need to be resized.
  Install this as a signal handler for resizing. The tricky part is that we need to call
  the default ncurses handler still, otherwise the resize does not work well.
