...
*** TODO Implement fuzzy search suggestions
ADDED: [2024-11-10 Sun]
...
+ Places where we want suggestions:
  + Entries screen
    + Renaming entry project
    + Renaming entry task
    + Renaming entry location
  + Stopwatch
    + Setting the stopwatch project.
    + Setting the stopwatch task.
    + Setting the stopwatch location.
