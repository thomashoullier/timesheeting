...
*** TODO Stay in stopwatch or register when resizing from the entries screen.
...
*** TODO Parse hh:mm using the currently selected register day
ADDED: [2025-02-28 Fri]
Use the currently selected register day to parse a date string given as just
"hh", "hh:mm", "hh:mm:ss".

*** TODO Do not update the stopwatch start date when commit fails
ADDED: [2025-03-05 Wed]
Currently, when a stopwatch commit is rejected, the start date is updated. Do nothing instead.
